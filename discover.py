...
class Character():
    ...
    def create_event(self,event_type):
        ...
        if event_type == 'enjoy':
            ...
            event_new = Event(datetime.now(),event_type='observation' ,flags=flags,verbs=verbs_f,adjectives=adjectives_f,loc=self.location)
...
compGeneralBuy = Flag_Component(       adjectives = ['affordable','pricy','cheap','common','democratic-priced','unexpensive','low-cost'],
...
def test_event_constructoritem(item ):
    ...
    for flag in flags:
        for comp in flag.flag_components:
            ...
            for adj in range(0,len(comp.adjectives)):
                adjectives_f.append(adj)
    event_dummy = Event(0,flags=flags,event_type='observation_item',verbs=verbs_f,adjectives=adjectives_f,nouns=[item.name],loc=None)
    return event_dummy
